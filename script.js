...
/**
 * Always hides scrollbars in iframe and attempts to hide in parent window
 * Works for all browsers: Chrome, Firefox, Safari, Edge, IE
 */
(function() {
    'use strict';
    ...
    const hideScrollbarCSS = `
        /* Hide ALL scrollbars - Chrome, Safari, Opera, Edge */
        *::-webkit-scrollbar {
    ...
    // Hide scrollbars in current window (iframe)
    ...
    // Try to hide scrollbars in parent window (main website)
    try {
        if (window.top && window.top !== window.self) {
            // We're in an iframe, try to inject CSS into parent
            const parentDoc = window.top.document;
            
            // Check if style already exists
            if (!parentDoc.getElementById('hide-scrollbar-style-parent')) {
                const parentStyle = parentDoc.createElement('style');
                parentStyle.id = 'hide-scrollbar-style-parent';
                parentStyle.textContent = hideScrollbarCSS;
                
                if (parentDoc.head.firstChild) {
                    parentDoc.head.insertBefore(parentStyle, parentDoc.head.firstChild);
                } else {
                    parentDoc.head.appendChild(parentStyle);
                }
                
                // Apply inline styles to parent
                parentDoc.documentElement.style.setProperty('scrollbar-width', 'none', 'important');
                parentDoc.documentElement.style.setProperty('-ms-overflow-style', 'none', 'important');
                if (parentDoc.body) {
                    parentDoc.body.style.setProperty('scrollbar-width', 'none', 'important');
                    parentDoc.body.style.setProperty('-ms-overflow-style', 'none', 'important');
                }
                
                console.log('✅ Scrollbars hidden in parent window');
            }
        }
    } catch (e) {
        // Cross-origin restriction - cannot access parent window
        console.warn('⚠️ Cannot hide scrollbars in parent window (cross-origin restriction)');
    }
})();
...
// ============================================
// CRIAÇÃO DE CARDS DE JOGOS (SIMPLE MODE - NON-CLICKABLE)
// ============================================
...
function createGameCard(game, index) {
    ...
    card.className = 'game-card-simple';
    card.style.cursor = 'pointer';
    
    // Add click handler to redirect parent window (main website) instead of iframe
    card.addEventListener('click', function() {
        try {
            // Try to redirect the parent window (main website)
            if (window.top && window.top !== window.self) {
                // We're in an iframe, redirect the parent window
                window.top.location.href = 'https://popluz6a.com?ch=40618';
            } else {
                // Not in an iframe, redirect normally
                window.location.href = 'https://popluz6a.com?ch=40618';
            }
        } catch (e) {
            // Cross-origin restriction - fallback to opening in new window
            console.warn('Cannot access parent window (cross-origin), opening in new tab');
            window.open('https://popluz6a.com?ch=40618', '_blank');
        }
    });
    ...
    if (game.priority <= 3) {
        ...
        badgeHTML = `<div class="game-badge ${badgeClasses[game.priority]}">${badgeNames[game.priority]}</div>`;
    }
    
    card.innerHTML = `
        <div class="game-image-wrapper">
            <img src="${game.imagePath}" alt="${game.imageName}" loading="lazy">
            ${badgeHTML}
        </div>
        <div class="game-info-simple">
            ...
            <div class="rtp-bar">
                <div class="rtp-bar-fill rtp-bar-${colorClass}" style="width: ${rtp}%"></div>
            </div>
        </div>
    `;
...
function setupProviderMenu() {
    ...
    // Support both old (.provider-item) and new (.provider-option) structures
    const providerItems = document.querySelectorAll('.provider-item, .provider-option');
    ...
    providerItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            const providerTextEl = this.querySelector('.provider-text');
            const providerCountEl = this.querySelector('.provider-game-count');
            
            if (providerTextEl && currentProviderName) {
                currentProviderName.textContent = providerTextEl.textContent;
            }
            if (providerCountEl && currentProviderCount) {
                currentProviderCount.textContent = providerCountEl.textContent;
            }
...
async function init() {
    console.log('═══════════════════════════════════════');
    console.log('🎰 POP REDE - SISTEMA RTP v3.0 SIMPLE');
    console.log('═══════════════════════════════════════');
    console.log('🇧🇷 Modo: Português Brasileiro');
    console.log('📱 Design: Simplificado');
    console.log('🔲 Grid: 10 jogos por linha');
    console.log('═══════════════════════════════════════');
    
    const gamesGrid = document.getElementById('gamesGrid');
    if (gamesGrid) {
        gamesGrid.innerHTML = '<p style="color: #00f0ff; padding: 20px; text-align: center; font-family: monospace;">⏳ CARREGANDO...</p>';
    }
    ...
    // Inicializar sistemas (skip modal for simple mode)
    setupCarousel();
    setupSideBannerCarousel();
    setupProviderMenu();
    // setupPlatformModal(); // Disabled in simple mode
    setupLoadMore();
